...
export const SelectStartDate: FC<{
  ...
  const [startDateRange, setStartDateRange] = useState(createStartDateList(selectedStartDate));
  ...
  const handleClickMoveDateRange = (moveType: MoveDateRange): undefined => {
    ...
    if (moveType === "prev") {
      moveDate = customDayjs(selectedStartDate).subtract(1, "week").format("YYYY-MM-DD");
    } else {
      moveDate = customDayjs(selectedStartDate).add(1, "week").format("YYYY-MM-DD");
    }
